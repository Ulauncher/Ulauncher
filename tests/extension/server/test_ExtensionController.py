...
from ulauncher.extension.shared.event import KeywordQueryEvent
...
class TestExtensionController:
    ...
    def resultRenderer(self, mocker):
        return mocker.patch(
            'ulauncher.extension.server.ExtensionController.DeferredResultRenderer.get_instance').return_value

    @pytest.fixture(autouse=True)
    def extPrefs(self, mocker):
        return mocker.patch(
            'ulauncher.extension.server.ExtensionController.ExtensionPreferences').return_value

    @pytest.fixture(autouse=True)
    def manifest(self, mocker):
        return mocker.patch(
            'ulauncher.extension.server.ExtensionController.ExtensionManifest.open').return_value
    ...
    def test_handleConnected__invalid_extension_id__raises(self, controller, controllers):
        controller.request = mock.Mock()
        controller.request.path = '/'
        with pytest.raises(Exception):
            controller.handleConnected()

    def test_handleConnected__preferences__sent_to_client(self, controller, controllers, extPrefs, mocker):
        extPrefs.get_dict.return_value = {}
        controller.request = mock.Mock()
        controller.request.path = '/extension-name'
        PreferencesEvent = mocker.patch(
            'ulauncher.extension.server.ExtensionController.PreferencesEvent')
    ...
    def test_handleMessage__handle_response__is_called(self, controller, resultRenderer, mocker):
        ...
        loads = mocker.patch('ulauncher.extension.server.ExtensionController.pickle.loads')
