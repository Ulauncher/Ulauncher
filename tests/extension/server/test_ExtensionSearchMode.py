...
class TestExtensionSearchMode:
    ...
    def extServer(self, mocker):
        return mocker.patch('ulauncher.extension.server.ExtensionSearchMode.ExtensionServer.get_instance').return_value

    @pytest.fixture(autouse=True)
    def resultRenderer(self, mocker):
        return mocker.patch(
            'ulauncher.extension.server.ExtensionSearchMode.DeferredResultRenderer.get_instance').return_value
