...
from ulauncher.extension.server.ExtensionServer import ExtensionServer, ServerIsRunningError
...
class TestExtensionServer:
    ...
    def find_unused_port(self, mocker):
        return mocker.patch('ulauncher.extension.server.ExtensionServer.find_unused_port')

    @pytest.fixture(autouse=True)
    def SWS(self, mocker):
        return mocker.patch('ulauncher.extension.server.ExtensionServer.SimpleWebSocketServer')
    ...
    def run_async(self, mocker):
        return mocker.patch('ulauncher.extension.server.ExtensionServer.run_async')
