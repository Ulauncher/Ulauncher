...
class TestExtensionRuntime:
    ...
    def SubprocessLauncher(self, mocker):
        return mocker.patch("ulauncher.modes.extensions.ExtensionRuntime.Gio.SubprocessLauncher")

    @pytest.fixture(autouse=True)
    def DataInputStream(self, mocker):
        return mocker.patch("ulauncher.modes.extensions.ExtensionRuntime.Gio.DataInputStream")

    @pytest.fixture
    def time(self, mocker):
        return mocker.patch("ulauncher.modes.extensions.ExtensionRuntime.time")

    def test_run__basic_execution__is_called(self, SubprocessLauncher):
