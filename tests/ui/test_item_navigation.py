...
class TestItemNavigation:
    ...
    def test_select_is_called(self, nav, items) -> None:
        nav.select(1)
        assert nav.index == 1
        items[1].select.assert_called_once_with()

    def test_select_and_deselect_is_called(self, nav, items) -> None:
        nav.select(1)
        nav.select(5)
    ...
    def test_go_up_from_start(self, nav, items) -> None:
        nav.go_up()
        items[4].select.assert_called_once_with()

    def test_go_up_from_1st(self, nav, items) -> None:
        nav.select(1)
        nav.go_up()
        items[0].select.assert_called_once_with()

    def test_go_up_from_last(self, nav, items) -> None:
        nav.select(4)
        nav.go_up()
        items[3].select.assert_called_once_with()

    def test_go_down_from_second(self, nav, items) -> None:
        nav.select(2)
        nav.go_down()
        items[3].select.assert_called_once_with()

    def test_go_down_from_last(self, nav, items) -> None:
        nav.select(4)
        nav.go_down()
        items[0].select.assert_called_once_with()
