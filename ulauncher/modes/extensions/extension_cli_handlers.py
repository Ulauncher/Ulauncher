...
def preview_extension(_: ArgumentParser, args: Namespace) -> bool:  # noqa: PLR0911, PLR0912, PLR0915 - intentionally single function for ease of maintenance
    ...
    result = cast("tuple[bool, str] | None", dbus_trigger_event("extensions:preview_ext", preview_message, wait=True))
    (is_error, error_msg) = result or (True, "No response from D-Bus")

    if is_error:
        logger.error("Error: Could not start preview: %s", error_msg)
        return False
