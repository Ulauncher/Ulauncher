...
class ExtensionRuntime:
    ...
    def __init__(
        ...
        launcher = Gio.SubprocessLauncher.new(GSPFlags.STDIN_PIPE | GSPFlags.STDOUT_PIPE | GSPFlags.STDERR_PIPE)
        ...
        if stdout_base_stream := self.subprocess.get_stdout_pipe():
            self.stdout = Gio.DataInputStream.new(stdout_base_stream)
    ...
    def read_stdout_line(self) -> None:
        self.stdout.read_line_async(GLib.PRIORITY_DEFAULT, None, self.handle_stdout)
    ...
    def handle_stdout(self, stdout: Gio.DataInputStream, result: Gio.AsyncResult) -> None:
        """Handle JSON messages from extension stdout."""
        output, _ = stdout.read_line_finish_utf8(result)
        if output:
            try:
                message = json.loads(output)
                logger.debug('Incoming response with keys "%s" from "%s"', set(message), self.ext_id)
                events.emit("extensions:handle_response", message)
            except json.JSONDecodeError:
                logger.warning("Invalid JSON from %s stdout: %s", self.ext_id, output)
            self.read_stdout_line()
