...
class PreferencesServer:
    ...
    async def extension_get_all(self, reload: bool) -> dict[str, dict[str, Any]]:
        ...
        if reload:
            tasks = [controller.start() for controller in extension_registry.iterate() if controller.is_enabled]
            await asyncio.gather(*tasks)
        return {ex.id: get_extension_data(ex) for ex in extension_registry.iterate()}
